...
class _AdminRegistrationScreenState extends State<AdminRegistrationScreen> {
  ...
  void didChangeDependencies() {
    ...
    _vm = AdminRegistrationViewModel(
  ...
  Widget build(BuildContext context) {
    final vm = _vm!;
    return Scaffold(
      appBar: AppBar(title: const Text('Approval Pendaftaran (Request)')),
                      ...
                      AdminSectionCard(
                        ...
                        child: vm.pending.isEmpty
                            ? const Padding(
                                padding: EdgeInsets.symmetric(vertical: 48),
                                child: Center(
                                  child: Text('Tidak ada request PENDING.'),
                                ),
                              )
                            : ListView.separated(
                                ...
                                itemBuilder: (context, i) => _ReqTile(
                                  item: vm.pending[i],
                                  onApprove: () =>
                                      _approve(context, vm, vm.pending[i]),
                                  onReject: () =>
                                      _reject(context, vm, vm.pending[i]),
                                ),
                              ),
                      ),
  ...
  Future<void> _reject(
    ...
    AdminRegistrationViewModel vm,
    RegistrationRequestDto item,
  ) async {
    final reason = await _askReason(
      context,
      title: 'Reject Request',
      hint: 'Alasan reject (wajib)',
    );
  ...
  Future<void> _approve(
    ...
    final res = await showDialog<_ApprovePayload>(
      context: context,
      builder: (context) =>
          _ApproveDialog(item: item, units: vm.units, mentors: vm.mentors),
    );
    if (res == null) return;
...
class _ApproveDialogState extends State<_ApproveDialog> {
  ...
  late final TextEditingController start = TextEditingController(
    text: widget.item.internshipStart,
  );
  late final TextEditingController end = TextEditingController(
    text: widget.item.internshipEnd,
  );
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            if ((widget.item.schoolName ?? '').isNotEmpty ||
                (widget.item.schoolAddress ?? '').isNotEmpty) ...[
              Align(
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: 'Password sementara (kosong = auto)',
              ),
            ),
            const SizedBox(height: 12),
            TextField(
