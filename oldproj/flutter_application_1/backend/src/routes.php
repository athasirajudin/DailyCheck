...
function route_request(mysqli $db): void {
  ...
  if ($method === 'POST' && $p === '/admin/pairing/create') {
    route_admin_create_pairing($db);
    return;
  }
  ...
  if ($method === 'GET' && $p === '/admin/devices') {
    route_admin_devices($db);
    return;
  }
  ...
  if ($method === 'POST' && $p === '/device/pair') {
    route_device_pair($db);
    return;
  }
  if ($method === 'POST' && $p === '/device/heartbeat') {
    route_device_heartbeat($db);
    return;
  }
  if ($method === 'POST' && $p === '/device/qr-token') {
    route_device_qr_token($db);
    return;
  }
  ...
  if ($method === 'POST' && $p === '/mentor/pairing/create') {
    route_mentor_create_pairing($db);
    return;
  }
...
function route_admin_save_settings(mysqli $db): void {
  ...
  db_exec(
    $db,
    'INSERT INTO settings (id, timezone, work_start_time, tolerance_minutes, day_cutoff_time, workdays_json, offline_threshold_seconds, qr_token_ttl_seconds, updated_at, updated_by_user_id)
     VALUES (1,?,?,?,?,?,?,?,NOW(),?)
    ...
    'ssissiii',
    [$timezone, $workStart, $tol, $cutoff, $workdaysJson, $offlineThreshold, $qrTtl, (int)$u['user_id']]
  );
  json_ok(get_settings($db));
}

function route_admin_create_pairing(mysqli $db): void {
  $u = require_user($db, ['ADMIN']);
  $body = json_body();
  require_fields($body, ['unitId', 'deviceName']);
  $unitId = (int)$body['unitId'];
  ...
  $expiresAt = now_jakarta()->modify('+10 minutes')->format('Y-m-d H:i:s');
  db_exec(
    $db,
    'INSERT INTO pairing_codes (unit_id, device_name, code, expires_at, created_by_user_id, created_at)
     VALUES (?,?,?,?,?,NOW())',
    'isssi',
    [$unitId, $deviceName, $code, $expiresAt, (int)$u['user_id']]
  );
  json_ok(['pairingCode' => $code, 'expiresAt' => $expiresAt]);
}

function route_admin_devices(mysqli $db): void {
  require_user($db, ['ADMIN']);
  $settings = get_settings($db);
  $offline = (int)($settings['offline_threshold_seconds'] ?? DEFAULT_OFFLINE_THRESHOLD_SECONDS);
  $now = now_jakarta();
  $devices = db_all($db, 'SELECT d.id, d.unit_id, u.name AS unit_name, d.name, d.last_seen_at FROM devices d JOIN units u ON u.id=d.unit_id ORDER BY d.id DESC');
  $out = [];
  foreach ($devices as $d) {
    $last = $d['last_seen_at'] ? new DateTimeImmutable($d['last_seen_at'], new DateTimeZone(APP_TIMEZONE)) : null;
...
function route_device_pair(mysqli $db): void {
  $body = json_body();
  require_fields($body, ['pairingCode']);
  $pairingCode = strtoupper(trim((string)$body['pairingCode']));
  $row = db_one(
    $db,
    'SELECT id, unit_id, device_name, expires_at, used_at FROM pairing_codes WHERE code=? ORDER BY id DESC LIMIT 1',
    's',
    [$pairingCode]
  );
  if (!$row) {
    json_error('INVALID_PAIRING', 'Pairing code tidak valid.', 400);
  }
  if ($row['used_at'] !== null) {
    json_error('INVALID_PAIRING', 'Pairing code sudah digunakan.', 400);
  }
  $expires = new DateTimeImmutable($row['expires_at'], new DateTimeZone(APP_TIMEZONE));
  if ($expires <= now_jakarta()) {
    json_error('INVALID_PAIRING', 'Pairing code sudah kedaluwarsa.', 400);
  }

  $authKey = random_token(24);
  $authHash = sha256($authKey);
  db_exec(
    $db,
    'INSERT INTO devices (unit_id, name, auth_key_hash, last_seen_at, created_at, updated_at)
     VALUES (?,?,?,NOW(),NOW(),NOW())',
    'iss',
    [(int)$row['unit_id'], $row['device_name'], $authHash]
  );
  $deviceId = (int)$db->insert_id;
  db_exec($db, 'UPDATE pairing_codes SET used_at=NOW(), used_by_device_id=? WHERE id=?', 'ii', [$deviceId, (int)$row['id']]);
  json_ok([
    'deviceId' => $deviceId,
...
function route_device_heartbeat(mysqli $db): void {
  $device = require_device($db);
  db_exec($db, 'UPDATE devices SET last_seen_at=NOW(), updated_at=NOW() WHERE id=?', 'i', [(int)$device['id']]);
  json_ok(['deviceId' => (int)$device['id'], 'ok' => true]);
}

function route_device_qr_token(mysqli $db): void {
  $device = require_device($db);
  $settings = get_settings($db);
  $ttl = (int)($settings['qr_token_ttl_seconds'] ?? DEFAULT_QR_TTL_SECONDS);

  db_exec($db, 'UPDATE qr_sessions SET active=0, deactivated_at=NOW() WHERE device_id=? AND active=1', 'i', [(int)$device['id']]);

  $token = strtoupper(substr(bin2hex(random_bytes(12)), 0, 24));
  $expiresAt = now_jakarta()->modify("+{$ttl} seconds")->format('Y-m-d H:i:s');
  db_exec($db, 'INSERT INTO qr_sessions (device_id, token, expires_at, active, created_at) VALUES (?,?,?,?,NOW())', 'issi', [(int)$device['id'], $token, $expiresAt, 1]);
  json_ok(['token' => $token, 'expiresAt' => $expiresAt]);
}
...
function route_intern_today(mysqli $db): void {
  ...
  json_ok([
    'date' => $today,
    'unit' => ['id' => (int)$intern['unit_id'], 'name' => $intern['unit_name']],
...
function route_attendance_check(mysqli $db): void {
  $u = require_user($db, ['INTERN']);
  $body = json_body();
  require_fields($body, ['qrToken', 'action', 'lat', 'lon']);
  ...
  $intern = db_one(
    $db,
    'SELECT i.unit_id, i.internship_start, i.internship_end, i.active FROM interns i WHERE i.user_id=? LIMIT 1',
    'i',
    [(int)$u['user_id']]
  );
  ...
  $qr = db_one(
    $db,
    'SELECT qs.id, qs.device_id, qs.expires_at, qs.active, d.unit_id, u.geofence_lat, u.geofence_lon, u.geofence_radius_m
     FROM qr_sessions qs
     JOIN devices d ON d.id=qs.device_id
     JOIN units u ON u.id=d.unit_id
     WHERE qs.token=? LIMIT 1',
    's',
    [$qrToken]
  );
  if (!$qr) {
    json_error('QR_INVALID', 'QR token tidak ditemukan.', 400);
  }
  if ((int)$qr['active'] !== 1) {
    json_error('QR_EXPIRED', 'QR token sudah tidak aktif.', 409);
  }
  $qrExpires = new DateTimeImmutable($qr['expires_at'], new DateTimeZone(APP_TIMEZONE));
  if ($qrExpires <= $now) {
    db_exec($db, 'UPDATE qr_sessions SET active=0, deactivated_at=NOW() WHERE id=?', 'i', [(int)$qr['id']]);
    json_error('QR_EXPIRED', 'QR token sudah kedaluwarsa.', 409);
  }

  $internUnitId = (int)$intern['unit_id'];
  $unitId = (int)$qr['unit_id'];
  if ($internUnitId !== $unitId) {
    json_error('UNIT_MISMATCH', 'QR token bukan untuk unit kamu.', 403);
  }

  $gfLat = (float)$qr['geofence_lat'];
  $gfLon = (float)$qr['geofence_lon'];
  $gfR = (int)$qr['geofence_radius_m'];
  ...
  $startDt = new DateTimeImmutable($date . ' ' . $workStart, new DateTimeZone(APP_TIMEZONE));
...
function route_leave_request(mysqli $db): void {
  ...
  db_exec(
    $db,
    'INSERT INTO leave_requests (intern_user_id, type, date_from, date_to, reason, status, created_at)
     VALUES (?,?,?,?,?,"PENDING",NOW())',
    'issss',
    [(int)$u['user_id'], $type, $dateFrom, $dateTo, $reason]
  );
  $id = (int)$db->insert_id;
...
function route_mentor_create_pairing(mysqli $db): void {
  $u = require_user($db, ['PEMBIMBING']);
  $body = json_body();
  require_fields($body, ['unitId', 'deviceName']);
  $unitId = (int)$body['unitId'];
  ...
  $allowed = db_one(
    $db,
    'SELECT 1 FROM interns WHERE mentor_user_id=? AND unit_id=? LIMIT 1',
    'ii',
    [(int)$u['user_id'], $unitId]
  );
  if (!$allowed) {
    json_error('FORBIDDEN', 'Kamu tidak punya intern di unit ini.', 403);
  }
