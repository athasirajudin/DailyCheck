...
class _AdminPairingScreenState extends State<AdminPairingScreen> {
  ...
  void didChangeDependencies() {
    ...
    _vm = AdminPairingViewModel(apiClient: scope.apiClient, session: scope.session);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: vm.loading ? null : () => vm.loadUnits(), icon: const Icon(Icons.refresh)),
        ],
      ),
          ...
          return ListView(
            ...
            children: [
              ...
              DropdownButtonFormField<UnitDto>(
                ...
                items: vm.units
                    .map((u) => DropdownMenuItem<UnitDto>(value: u, child: Text('${u.id} • ${u.name}')))
                    .toList(),
                onChanged: vm.loading ? null : (v) => setState(() => _selectedUnit = v),
              ),
              ...
              SizedBox(
                ...
                child: FilledButton.icon(
                  onPressed: vm.loading || _selectedUnit == null
                      ? null
                      : () => vm.createPairing(unitId: _selectedUnit!.id, deviceName: _deviceName.text.trim()),
                  icon: const Icon(Icons.link),
                  label: vm.loading ? const Text('Memproses...') : const Text('Generate Pairing Code'),
                ),
              ),
