...
class _MentorPairingScreenState extends State<MentorPairingScreen> {
  ...
  Future<void> _loadUnits() async {
    ...
    try {
      final data = await scope.apiClient.getJson('/api/mentor/units', bearerToken: scope.session.token);
      if (data is! List) throw ApiError(code: 'BAD_RESPONSE', message: 'Format unit tidak valid.');
      final units = data.whereType<Map>().map((e) => _UnitMini.fromJson(Map<String, dynamic>.from(e))).toList();
      setState(() {
        _units = units;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Pairing Display (Pembimbing)'),
        actions: [
          IconButton(onPressed: _loading ? null : _loadUnits, icon: const Icon(Icons.refresh)),
        ],
      ),
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          const Text('Pembimbing bisa membuat pairing code untuk unit intern bimbingannya.'),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SelectableText(_pairingCode!, style: Theme.of(context).textTheme.headlineMedium),
                    const SizedBox(height: 8),
...
class _UnitMini {
  ...
  factory _UnitMini.fromJson(Map<String, dynamic> json) {
    return _UnitMini(id: (json['id'] as num).toInt(), name: (json['name'] ?? '').toString());
  }
}
